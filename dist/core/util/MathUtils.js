 ...
 */
export function weightedSample(items, rng, weights) {
 ...
 * @returns Noise value in range [-1, 1]
 */
export function noise2D(x, y, scale = 1.0) {
    const X = Math.floor(x * scale) & 255;
    const Y = Math.floor(y * scale) & 255;
    x -= Math.floor(x * scale);
    y -= Math.floor(y * scale);
    ...
    const B = p[X + 1] + Y;
    // Gradient contributions
    const result = lerp(v, lerp(u, grad(p[A], x, y, 0), grad(p[B], x - 1, y, 0)), lerp(u, grad(p[A + 1], x, y - 1, 0), grad(p[B + 1], x - 1, y - 1, 0)));
    return result;
}
/**
 * Alias for noise2D for backward compatibility
 */
export function perlin2D(x, y, scale = 1.0) {
