...
export class CandleGenerator extends BaseObjectGenerator {
    constructor() {
        super(...arguments);
        this.defaultParams = {
            style: 'pillar',
    ...
    createCandleBody(config) {
        let geometry;
        const effectiveHeight = config.height * (1 - config.burnLevel * 0.3);
        if (config.style === 'jar') {
            // Jar candle - cylinder with glass container
            const jarGeometry = new THREE.CylinderGeometry(config.radius + 0.003, config.radius + 0.003, effectiveHeight * 0.7, 16);
            ...
            geometry = new THREE.CylinderGeometry(topRadius, config.radius, effectiveHeight, 16);
        }
    ...
    createHolder(config) {
        ...
        if (config.holderStyle === 'simple') {
            holderGeometry = new THREE.CylinderGeometry(config.radius + 0.02, config.radius + 0.02, 0.02, 16);
            ...
            holderGeometry = new THREE.TorusGeometry(config.radius + 0.025, 0.005, 16, 32);
            holderMaterial = new THREE.MeshStandardMaterial({
                color: 0xFFD700,
                metalness: 0.9,
                roughness: 0.1
            ...
            holderGeometry = new THREE.BoxGeometry(config.radius * 3, config.height * 0.8, config.radius * 3);
            holderMaterial = new THREE.MeshStandardMaterial({
                color: 0x2F2F2F,
                metalness: 0.7,
                roughness: 0.3,
...
//# sourceMappingURL=CandleGenerator.js.map