 ...
 */
import { Group, Mesh, PlaneGeometry, CylinderGeometry, TorusGeometry, MeshStandardMaterial } from 'three';
import { BaseObjectGenerator } from '../BaseObjectGenerator';
import { NoiseUtils } from '../../utils/NoiseUtils';
export class CurtainGenerator extends BaseObjectGenerator {
    constructor() {
        super();
        this.defaultParams = {
            style: 'drapes',
    ...
    createCurtainPanels(group, params) {
        const material = this.getMaterialByType(params.materialType, params.pattern);
        // Create folded curtain geometry
        const geometry = this.createFoldedCurtainGeometry(params.width, params.height, params.folds, params.style);
        const curtain = new Mesh(geometry, material);
        curtain.position.y = params.height / 2;
        group.add(curtain);
    }
    createFoldedCurtainGeometry(width, height, folds, style) {
    ...
    createSwagValanceGeometry(width, height, depth) {
        const segments = 20;
    ...
    getMaterialByType(type, pattern) {
        let baseColor;
        let roughness;
        let metalness;
    ...
    getVariations() {
        const styles = ['drapes', 'sheer', 'valance', 'cafe', 'pencil', 'grommet', 'rod_pocket'];
        const materials = ['cotton', 'linen', 'silk', 'velvet', 'polyester', 'lace'];
        const patterns = ['solid', 'striped', 'floral', 'geometric', 'damask'];
...
//# sourceMappingURL=CurtainGenerator.js.map