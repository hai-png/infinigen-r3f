...
export class VaseGenerator extends BaseObjectGenerator {
    constructor() {
        super(...arguments);
        this.defaultParams = {
            shape: 'cylinder',
    ...
    createCylinderVaseGeometry(config) {
        return new THREE.CylinderGeometry(config.radius, config.baseRadius, config.height, 32, 1, true);
    }
    createSphereVaseGeometry(config) {
    ...
    createConeVaseGeometry(config) {
        return new THREE.CylinderGeometry(config.neckRadius, config.baseRadius, config.height, 32, 1, true);
    }
    createHourglassVaseGeometry(config) {
        const points = [];
        const segments = 20;
    ...
    createAmphoraVaseGeometry(config) {
        const points = [];
        const segments = 25;
        for (let i = 0; i <= segments; i++) {
            const t = i / segments;
    ...
    createFluteVaseGeometry(config) {
        const points = [];
        const segments = 20;
        for (let i = 0; i <= segments; i++) {
            const t = i / segments;
    ...
    createSingleHandle(config, angle) {
        let handleShape;
    ...
    createNotchedRimGeometry(config) {
        const points = [];
        const notches = 8;
    ...
    createBase(config) {
        ...
        switch (config.baseStyle) {
            case 'rounded':
                ...
                break;
            case 'pedestal':
                baseGeometry = new THREE.CylinderGeometry(config.baseRadius * 0.5, config.baseRadius * 0.7, 0.05, 16);
                break;
            case 'wide':
                baseGeometry = new THREE.CylinderGeometry(config.baseRadius * 1.3, config.baseRadius * 1.5, 0.03, 16);
                break;
            default:
...
//# sourceMappingURL=VaseGenerator.js.map