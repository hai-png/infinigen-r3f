...
import { Node, RelationNode, Variable } from '../../language/types';
...
export interface SymmetryResult {
  ...
  symmetryPlane?: { normal: THREE.Vector3; point: THREE.Vector3 };
 ...
 */
export const symmetryNodeImpls = new Map<string, Function>();
 ...
 */
export function evaluateSymmetric(
  node: RelationNode,
  state: State,
  ...
  let symmetryAxis: THREE.Vector3 | undefined;
  let symmetryPlane: { normal: THREE.Vector3; point: THREE.Vector3 } | undefined;
  ...
  const symmetricPairs: Array<[string, string]> = [];
  let totalLoss = 0;
  const used = new Set<string>();
  
  for (let i = 0; i < objectIds.length; i++) {
    if (used.has(objectIds[i])) continue;
    ...
    for (let j = i + 1; j < objectIds.length; j++) {
      if (used.has(objectIds[j])) continue;
 ...
 * @param kwargs - Additional parameters
 */
export function evaluateAligned(
  node: RelationNode,
  state: State,
  childVals: Map<string, any>,
  kwargs: any = {}
): { satisfied: boolean; loss: number; direction?: THREE.Vector3 } {
  const args = node.args;
 ...
 * @param kwargs - Additional parameters
 */
export function evaluateDistributed(
  node: RelationNode,
  state: State,
  childVals: Map<string, any>,
  kwargs: any = {}
): { satisfied: boolean; loss: number; distribution?: 'uniform' | 'grid' | 'random' } {
  const args = node.args;
 ...
 */
function extractObjectIds(args: any[], childVals: Map<string, any>, state: State): string[] {
  const ids: string[] = [];
 ...
 */
function reflectAcrossAxis(point: THREE.Vector3, axis: THREE.Vector3): THREE.Vector3 {
 ...
 * Infer symmetry from object arrangement
 */
function inferSymmetryFromObjects(
  objectIds: string[],
  state: State
): { axis?: THREE.Vector3; plane?: { normal: THREE.Vector3; point: THREE.Vector3 } } {
  const centers = objectIds
    .map(id => state.getObject(id)?.getBBoxCenter())
    .filter((c): c is THREE.Vector3 => c !== undefined);
 ...
 */
function computeCovarianceMatrix(points: THREE.Vector3[]): THREE.Matrix3 {
  if (points.length === 0) {
 ...
 */
function computeEigenvectors(matrix: THREE.Matrix3): THREE.Vector3[] {
 ...
 */
function fitLineToPoints(points: THREE.Vector3[]): { point: THREE.Vector3; direction: THREE.Vector3 } {
  if (points.length === 0) {
 ...
 */
function distancePointToLine(point: THREE.Vector3, linePoint: THREE.Vector3, lineDir: THREE.Vector3): number {
