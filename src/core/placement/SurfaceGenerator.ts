...
export class SurfaceGenerator {
    ...
    switch (params.type) {
      case SurfacePrimitive.PLANE:
        geometry = new PlaneGeometry(
          params.width || 1,
          params.height || 1,
          params.segments || this.config.resolution,
          params.segments || this.config.resolution
        );
        break;
      ...
      case SurfacePrimitive.TORUS:
        geometry = new TorusGeometry(
          ...
          params.tube || 0.1,
          params.radialSegments || this.config.resolution,
          params.tubularSegments || this.config.resolution,
   ...
   */
  applyDisplacement(geometry: BufferGeometry, intensity: number = 1.0): BufferGeometry {
   ...
   */
  private generateDisplacementField(geometry: BufferGeometry): DisplacementField {
    const positions = geometry.attributes.position.array as Float32Array;
  ...
  private calculateGradientX(u: number, v: number, w: number, delta: number): number {
  ...
  private calculateGradientY(u: number, v: number, w: number, delta: number): number {
  ...
  private noise3D(x: number, y: number, z: number): number {
    ...
    const res = this.lerp(
      ...
      this.lerp(
        ...
        this.lerp(
          u,
          this.grad(this.pHash(AA), x, y, z),
          this.grad(this.pHash(BA), x - 1, y, z)
        ),
        this.lerp(
          u,
          this.grad(this.pHash(AB), x, y - 1, z),
          this.grad(this.pHash(BB), x - 1, y - 1, z)
        )
      ),
      this.lerp(
        v,
        this.lerp(
          u,
          this.grad(this.pHash(AA + 1), x, y, z - 1),
          this.grad(this.pHash(BA + 1), x - 1, y, z - 1)
        ),
        this.lerp(
          u,
          this.grad(this.pHash(AB + 1), x, y - 1, z - 1),
          this.grad(this.pHash(BB + 1), x - 1, y - 1, z - 1)
        )
      )
    );
  ...
  generateSurface(
    primitiveParams: SurfacePrimitiveParams,
    applyDisplacement: boolean = true,
    displacementIntensity: number = 1.0
  ): Mesh {
  ...
  private createDefaultMaterial(): any {
    return {
  ...
  generateExtrudedSurface(shape: Shape, depth: number = 1): Mesh {
  ...
  private applyPeriodicBoundary(geometry: BufferGeometry): void {
    console.log('Applying periodic boundary conditions');
  }

  private closeBoundaries(geometry: BufferGeometry): void {
