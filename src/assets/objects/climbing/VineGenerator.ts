...
/**
 * Configuration for vine generation
 */
...
/**
 * Generator for climbing and hanging vines with growth simulation
 */
export class VineGenerator {
  private readonly defaultConfig: VineConfig = {
    species: 'ivy',
  ...
  private generateClimbingStems(config: VineConfig): THREE.Mesh[] {
    ...
    for (let i = 0; i < stemCount; i++) {
      ...
      for (let s = 0; s < segments; s++) {
        ...
        const wanderX = NoiseUtils.perlin2D(t * noiseScale, i) * 0.1;
        const wanderZ = NoiseUtils.perlin2D(t * noiseScale, i + 100) * 0.1;
  ...
  private createWisteriaLeafGeometry(size: number): THREE.BufferGeometry {
    ...
    for (let i = 0; i < leafletCount; i++) {
      const leaflet = new THREE.Mesh(leafletGeometry);
      leaflet.position.y = (i - leafletCount / 2) * size * 0.2;
      group.add(leaflet);
    }

    // Simplified - return single leaflet geometry
  ...
  private createWisteriaFlowerGeometry(): THREE.BufferGeometry {
    ...
    for (let i = 0; i < flowerCount; i++) {
      const flower = new THREE.Mesh(flowerGeometry);
      flower.position.y = i * 0.03;
      group.add(flower);
    }
