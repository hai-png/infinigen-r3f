...
export class ShelfGenerator extends BaseObjectGenerator<ShelfParams> {
  ...
  private createShelves(params: ShelfParams, rng: SeededRandom): THREE.Group {
    ...
    for (let i = 1; i <= params.shelfCount; i++) {
      ...
      const shelfGeom = new THREE.BoxGeometry(
        params.width - (params.shelfType === 'ladder' ? 0.1 : 0.08),
        shelfThickness,
        effectiveDepth - (params.shelfType === 'ladder' ? 0.05 : 0)
      );
  ...
  getVariationCount(): number { return 4 * 5 * 4 * 2 * 2 * 2; }
  register(): void { ObjectRegistry.register(ShelfGenerator.GENERATOR_ID, this); }
}

if (typeof window !== 'undefined') { new ShelfGenerator().register(); }
