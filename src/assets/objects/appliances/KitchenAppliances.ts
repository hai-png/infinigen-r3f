...
export class KitchenAppliances extends ApplianceBase<KitchenApplianceParams> {
  ...
  public generate(params: Partial<KitchenApplianceParams> = {}): Group {
    const finalParams = this.validateAndMergeParams(params) as KitchenApplianceParams;
  ...
  private generateRefrigerator(params: KitchenApplianceParams): Group {
    const group = new Group();
    ...
    if (params.style === 'modern') {
      ...
      // Left door (freezer)
      const leftDoor = this.createDoor(
        params,
        { x: -width * 0.25, y: height * 0.25, z: 0 },
        { w: width * 0.45, h: doorHeight }
      );
      group.add(leftDoor);

      // Right door (fridge)
      const rightDoor = this.createDoor(
        params,
        { x: width * 0.25, y: height * 0.25, z: 0 },
        { w: width * 0.45, h: doorHeight }
      );
      ...
      // Top door
      const topDoor = this.createDoor(
        params,
        { x: 0, y: height * 0.375, z: 0 },
        { w: width, h: topDoorHeight }
      );
      group.add(topDoor);

      // Bottom door
      const bottomDoor = this.createDoor(
        params,
        { x: 0, y: -height * 0.175, z: 0 },
        { w: width, h: bottomDoorHeight }
      );
  ...
  private generateStove(params: KitchenApplianceParams): Group {
    const group = new Group();
  ...
  private generateDishwasher(params: KitchenApplianceParams): Group {
    const group = new Group();
  ...
  private generateMicrowave(params: KitchenApplianceParams): Group {
    const group = new Group();
  ...
  private createCooktop(params: KitchenApplianceParams, width: number, depth: number): Group {
    const group = new Group();
  ...
  private createControlPanel(params: KitchenApplianceParams, width: number): Group {
    const group = new Group();
    ...
    for (let i = 0; i < knobCount; i++) {
      const knob = new Mesh(
        new CylinderGeometry(0.02, 0.02, 0.03, 16),
        this.getHandleMaterial('stainless')
      );
  ...
  private createOvenDoor(params: KitchenApplianceParams, width: number, height: number): Group {
    const group = new Group();
  ...
  private createBackguard(params: KitchenApplianceParams, width: number): Group {
    const group = new Group();
  ...
  private addGasBurners(group: Group, params: KitchenApplianceParams, width: number, count: number): void {
    const burnerPositions = this.getBurnerPositions(count, width);
    
    burnerPositions.forEach(pos => {
      const burner = new Mesh(
        new TorusGeometry(0.06, 0.01, 8, 24),
        this.createPBRMaterial({ color: 0x333333, metalness: 0.8, roughness: 0.3 })
      );
  ...
  private addElectricCoils(group: Group, params: KitchenApplianceParams, width: number, count: number): void {
    const burnerPositions = this.getBurnerPositions(count, width);
    
    burnerPositions.forEach(pos => {
      const coil = new Mesh(
        new TorusGeometry(0.07, 0.008, 8, 32),
        this.createPBRMaterial({ color: 0x222222, metalness: 0.6, roughness: 0.4 })
      );
  ...
  private getBurnerPositions(count: number, width: number): Array<{x: number, z: number}> {
  ...
  private createBurnerGrate(): Group {
    const group = new Group();
  ...
  private createDrawer(params: KitchenApplianceParams, width: number, height: number): Group {
    const group = new Group();
  ...
  private createIceDispenser(): Group {
    const group = new Group();
  ...
  private createMicrowaveDoor(params: KitchenApplianceParams, width: number, height: number): Group {
    const group = new Group();
  ...
  private createMicrowaveControls(params: KitchenApplianceParams): Group {
    const group = new Group();
  ...
  private createControlStrip(params: KitchenApplianceParams, width: number): Group {
    const group = new Group();
  ...
  private createKickPlate(params: KitchenApplianceParams, width: number): Group {
    const group = new Group();
  ...
  private addRefrigeratorHandles(group: Group, params: KitchenApplianceParams, width: number, height: number, depth: number): void {
  ...
  public getRandomParams(): KitchenApplianceParams {
    const types = ['refrigerator', 'stove', 'dishwasher', 'microwave'] as const;
