/**
 * Lamp Generator - Articulated desk/floor lamp with adjustable arm
 */
...
export class LampGenerator extends ArticulatedObjectBase {
  ...
  generate(config?: Partial<ArticulatedObjectConfig>): ArticulatedObjectResult {
    ...
    meshGeometries.set('lamp_shade', { size: new THREE.Vector3(0.07, 0.06, 0.07), pos: new THREE.Vector3(0, 0.65, 0) });

    return { group, joints, category: this.category, config: cfg, toMJCF: () => generateMJCF('lamp', joints, meshGeometries) };
  }
}
