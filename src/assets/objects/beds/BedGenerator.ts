...
export class BedGenerator extends BaseObjectGenerator<BedParams> {
  ...
  protected validateParams(params: Partial<BedParams>): Partial<BedParams> {
    const validated: Partial<BedParams> = { ...params };
  ...
  public generate(params: Partial<BedParams> = {}): Group {
    const finalParams = this.validateAndMergeParams(params) as BedParams;
  ...
  private getBedDimensions(size: string): { width: number; length: number } {
  ...
  private createFrame(dimensions: { width: number; length: number }, params: BedParams): Group {
    const group = new Group();
  ...
  private createHeadboard(dimensions: { width: number; length: number }, params: BedParams): Group {
    const group = new Group();
    const material = this.getFrameMaterial(params.frameMaterial, params.style);
  ...
  private createFootboard(dimensions: { width: number; length: number }, params: BedParams): Group {
    const group = new Group();
    const material = this.getFrameMaterial(params.frameMaterial, params.style);
  ...
  private createMattress(dimensions: { width: number; length: number }, thickness: number): Mesh {
  ...
  private createBedding(dimensions: { width: number; length: number }, params: BedParams): Group {
    const group = new Group();
    ...
    if (params.beddingStyle === 'duvet') {
      ...
      for (let i = 0; i < 4; i++) {
        for (let j = 0; j < 6; j++) {
          ...
          quilt.position.set(
            -dimensions.width / 2 + 0.2 + i * 0.35,
            0.125,
            -beddingLength / 2 + 0.2 + j * 0.25
          );
          group.add(quilt);
        }
      }
  ...
  private createPillows(dimensions: { width: number; length: number }, count: number, style: string): Group {
    const group = new Group();
  ...
  private createStorageDrawers(dimensions: { width: number; length: number }, params: BedParams): Group {
    const group = new Group();
    ...
    [-1, 1].forEach(side => {
      for (let i = 0; i < 2; i++) {
        ...
        drawer.position.set(
          side * (dimensions.width / 2 + drawerWidth / 2 + 0.05),
          0.1,
          -dimensions.length / 4 + i * 0.3
        );
        ...
        handle.position.set(
          side * (dimensions.width / 2 + drawerWidth + 0.06),
          0.1,
          -dimensions.length / 4 + i * 0.3
        );
  ...
  private createCanopy(dimensions: { width: number; length: number }, params: BedParams): Group {
    const group = new Group();
    const material = this.getFrameMaterial(params.frameMaterial, params.style);
  ...
  private getFrameMaterial(frameMaterial: string, style: string): any {
  ...
  public getBoundingBox(params: BedParams): BBox {
  ...
  public getRandomParams(): BedParams {
    const sizes = ['twin', 'full', 'queen', 'king', 'cal-king'] as const;
    const styles = ['modern', 'traditional', 'minimal', 'canopy', 'storage'] as const;
