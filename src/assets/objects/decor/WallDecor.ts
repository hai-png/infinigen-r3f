...
export class WallDecor extends BaseObjectGenerator<WallDecorParams> {
  ...
  protected validateParams(params: Partial<WallDecorParams>): Partial<WallDecorParams> {
    const validated: Partial<WallDecorParams> = { ...params };
    
    if (validated.width !== undefined) validated.width = Math.max(0.2, Math.min(2.0, validated.width));
    if (validated.height !== undefined) validated.height = Math.max(0.2, Math.min(2.0, validated.height));
    
    return validated;
  }

  public generate(params: Partial<WallDecorParams> = {}): Group {
    const finalParams = this.validateAndMergeParams(params) as WallDecorParams;
  ...
  private createContent(params: WallDecorParams): Mesh {
    let geometry: any;
    let material: any;
  ...
  private createFrame(params: WallDecorParams): Group {
    const group = new Group();
  ...
  private createGlass(params: WallDecorParams): Mesh {
    const { width, height, shape } = params;
  ...
  private createHangingHardware(params: WallDecorParams): Group {
    const group = new Group();
  ...
  private createAbstractShape(width: number, height: number): Shape {
  ...
  private createClockFace(width: number, height: number): any {
  ...
  private createArtworkMaterial(style: string, width: number, height: number): any {
  ...
  private createPicturePlaceholder(style: string, width: number, height: number): any {
    ...
    return this.createPBRMaterial({
      color: matColors[style as keyof typeof matColors] || 0xffffff,
      metalness: 0.0,
      roughness: 0.95,
    });
  }

  private getFrameMaterial(material: string, style: string): any {
    switch (material) {
      case 'wood':
        return this.createPBRMaterial({
  ...
  public getRandomParams(): WallDecorParams {
    const types = ['picture', 'mirror', 'art', 'clock'] as const;
    const styles = ['modern', 'traditional', 'minimal', 'industrial', 'bohemian'] as const;
    const materials = ['wood', 'metal', 'plastic', 'none'] as const;
