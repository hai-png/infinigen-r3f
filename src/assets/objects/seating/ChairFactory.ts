...
export class ChairFactory extends AssetFactory {
  ...
  private static readonly BACK_TYPES = ['whole', 'partial', 'horizontal-bar', 'vertical-bar'] as const;
  ...
  protected generateConfig(): ChairConfig {
    ...
    const legType = weightedSample(['vertical', 'straight', 'up-curved', 'down-curved']) as ChairConfig['legType'];
    
    // Initialize offsets
    let legXOffset = 0;
    let legYOffset: [number, number] = [0, 0];
    let backXOffset = 0;
    let backYOffset = 0;
    ...
    const legOffsetBar: [number, number] = [rng.uniform(0.2, 0.4), rng.uniform(0.6, 0.8)];
    ...
    const armMid = new THREE.Vector3(
      ...
      rng.uniform(-0.09, 0.03)
    );
    const armProfile = [rng.logUniform(0.1, 3)];
    
    // Back parameters
    const backThickness = rng.uniform(0.04, 0.05);
    const backType = weightedSample(ChairFactory.BACK_TYPES) as ChairConfig['backType'];
    const backProfile: [number, number][] = [[0, 1]];
  ...
  protected makeSeat(): THREE.Mesh {
    ...
    // Create base curve
    const curve = this.curveGenerator.createBezierCurve(
      xAnchors, yAnchors, zAnchors, vectorLocations
    );
  ...
  protected makeLegs(): THREE.Mesh[] {
    ...
    const legEnds = legStarts.map((start, i) => {
      ...
      if (i === 0 || i === 1) end.x -= c.legXOffset;
      if (i === 2 || i === 3) end.x += c.legXOffset;
      if (i === 0 || i === 3) end.y += c.legYOffset[0];
      if (i === 1 || i === 2) end.y -= c.legYOffset[1];
      end.z = -c.legHeight;
      return end;
    });
  ...
  protected makeLimbs(ends: THREE.Vector3[], starts: THREE.Vector3[]): THREE.Mesh[] {
    ...
    for (let i = 0; i < starts.length; i++) {
      ...
      let axes: [number, number, number][] | null = null;
      ...
      const locationOffset = new THREE.Vector3(
        end.x < 0 ? 1 : -1,
        end.y < -c.size / 2 ? 1 : -1,
        0
      ).multiplyScalar(c.legThickness / 2);
  ...
  protected makeArms(base: THREE.Mesh, backs: THREE.Mesh[]): THREE.Mesh[] {
    ...
    if (!c.hasArm || backs.length === 0) return arms;
