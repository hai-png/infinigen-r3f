...
export class OutdoorLightGenerator {
  ...
  generate(params: Partial<OutdoorLightParams> = {}): OutdoorLightResult {
    const finalParams: OutdoorLightParams = {
   ...
   */
  private createPole(params: OutdoorLightParams): THREE.Mesh {
    const geometry = new THREE.CylinderGeometry(
      params.poleThickness * 0.8,
      params.poleThickness,
      params.height,
      8,
      1
    );
   ...
   */
  private createFixture(params: OutdoorLightParams, index: number = 0): { mesh: THREE.Group; lights: THREE.Light[] } {
    const group = new THREE.Group();
    const lights: THREE.Light[] = [];
   ...
   */
  private createArm(params: OutdoorLightParams): THREE.Mesh {
   ...
   * Create lamp housing
   */
  private createHousing(params: OutdoorLightParams): THREE.Mesh {
    let geometry: THREE.BufferGeometry;
   ...
   * Create actual light source
   */
  private createLightSource(params: OutdoorLightParams): THREE.Object3D {
    const group = new THREE.Group();
   ...
   */
  private createLens(params: OutdoorLightParams): THREE.Mesh {
   ...
   */
  private createSolarPanel(params: OutdoorLightParams): THREE.Mesh {
   ...
   */
  private createDecorations(params: OutdoorLightParams): THREE.Group {
    const group = new THREE.Group();

    if (params.style === 'vintage' || params.style === 'decorative') {
      // Add ornamental rings
      const ringGeometry = new THREE.TorusGeometry(
        params.poleThickness * 1.3,
        0.02,
        8,
        16
      );
   ...
   */
  private getPoleMaterial(materialType: string, style: LightStyle): THREE.Material {
    switch (materialType) {
      case 'wood':
   ...
   */
  private getArmEndPosition(params: OutdoorLightParams): THREE.Vector3 {
    const armLength = params.type === 'street' ? 1.5 : 0.8;
   ...
   */
  generateStreetLightRow(
    count: number,
    spacing: number,
    params: Partial<OutdoorLightParams> = {}
  ): THREE.Group {
    const group = new THREE.Group();
   ...
   */
  generatePathwayLights(
    length: number,
    spacing: number,
    params: Partial<OutdoorLightParams> = {}
  ): THREE.Group {
    const finalParams: Partial<OutdoorLightParams> = {
      ...params,
