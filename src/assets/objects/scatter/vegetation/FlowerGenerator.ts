...
export class FlowerGenerator extends BaseObjectGenerator<FlowerConfig> {
  ...
  private createPetals(config: FlowerConfig, rng: SeededRandom): THREE.Group {
    const group = new THREE.Group();
  ...
  private createCenter(config: FlowerConfig): THREE.Mesh {
