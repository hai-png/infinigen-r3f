 ...
 *
 * Also provides a CreatureBase-based adapter (CrustaceanAdapter) for backward compatibility.
 *
 * CANONICAL location for CrustaceanGenerator. Previously in vegetation/crustacean/.
 *
 * @module creatures
 */
...
import { CreatureSkinSystem, type CreatureSkinConfig } from './skin/CreatureSkinSystem';
...
/** @deprecated Use CrustaceanConfig instead */
...
// ── CrustaceanGenerator (Canonical Implementation) ────────────────────
...
/**
 * Adapter that wraps CrustaceanGenerator in the CreatureBase API.
 * Preserves backward compatibility for code using the CreatureBase-based API.
 *
 * Previously named CrustaceanGenerator in this file; renamed to avoid
 * collision with the canonical implementation class.
 */
export class CrustaceanAdapter extends CreatureBase {
  ...
  /**
   * Generate a crustacean mesh group.
   * Maps legacy CrustaceanParams to CrustaceanConfig and delegates to CrustaceanGenerator.
   *
  ...
  generate(speciesOrParams: CrustaceanSpecies | Partial<CrustaceanParams> = 'crab', params?: Partial<CrustaceanParams>): Group {
    ...
    const generator = new CrustaceanGenerator(mergedParams.seed);
    return generator.generate(config);
  }
