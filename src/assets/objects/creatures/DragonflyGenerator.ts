 ...
 *
 * Also provides a CreatureBase-based adapter (DragonflyAdapter) for backward compatibility.
 *
 * CANONICAL location for DragonflyGenerator. Previously in vegetation/dragonfly/.
 *
 * @module creatures
 */

import {
  Object3D, Group, Mesh, Material, MeshPhysicalMaterial, MeshStandardMaterial, DoubleSide,
...
/** @deprecated Use DragonflyConfig instead */
...
// ── DragonflyGenerator (Canonical Implementation) ─────────────────────
...
// ── CreatureBase Adapter (backward compatibility) ─────────────────────

/**
 * Adapter that wraps DragonflyGenerator in the CreatureBase API.
 * Preserves backward compatibility for code using the CreatureBase-based API.
 *
 * Previously named DragonflyGenerator in this file; renamed to avoid
 * collision with the canonical implementation class.
 */
export class DragonflyAdapter extends CreatureBase {
  ...
  /**
   * Generate a dragonfly mesh group.
   * Maps legacy DragonflyParams to DragonflyConfig and delegates to DragonflyGenerator.
   */
  generate(params: Partial<DragonflyParams> = {}): Group {
    ...
    // Map legacy DragonflyParams → DragonflyConfig
    ...
    const generator = new DragonflyGenerator(this._params.seed);
    return generator.generate(config);
  }
